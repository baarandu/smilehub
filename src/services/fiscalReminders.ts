...
export const fiscalRemindersService = {
    ...
    async getFiscalAlerts(
        ...
        const existingSet = new Set(
            (existingDocs || []).map(d => `${d.category}:${d.subcategory}`)
        );
    ...
    // Create default reminders for a tax regime
    async createDefaultReminders(clinicId: string, taxRegime: TaxRegime, fiscalYear: number): Promise<void> {
        const defaultReminders: Omit<FiscalReminder, 'id' | 'created_at' | 'updated_at'>[] = [];
            ...
            defaultReminders.push(
                {
                    ...
                    tax_regime: 'pf',
                    category: 'impostos',
                    subcategory: 'carne_leao',
                    title: 'Carnê-Leão mensal',
                    description: 'Prazo para pagamento do Carnê-Leão',
                    due_date: `${fiscalYear}-${String(new Date().getMonth() + 2).padStart(2, '0')}-28`,
                    frequency: 'monthly',
                    is_active: true,
                },
                {
                    ...
                    subcategory: null,
                    title: 'Prazo entrega IRPF',
                    description: 'Último dia para entrega da declaração de IR',
                    due_date: `${fiscalYear + 1}-04-30`,
                    frequency: 'annually',
                    is_active: true,
                }
            );
            ...
            defaultReminders.push(
                {
                    ...
                    tax_regime: 'simples',
                    category: 'impostos',
                    subcategory: 'das_simples',
                    title: 'DAS Simples Nacional',
                    description: 'Prazo para pagamento do DAS',
                    due_date: `${fiscalYear}-${String(new Date().getMonth() + 2).padStart(2, '0')}-20`,
                    frequency: 'monthly',
                    is_active: true,
                },
                {
                    ...
                    category: 'especificos',
                    subcategory: 'defis',
                    title: 'Prazo entrega DEFIS',
                    description: 'Último dia para entrega da DEFIS',
                    due_date: `${fiscalYear + 1}-03-31`,
                    frequency: 'annually',
                    is_active: true,
                }
            );
