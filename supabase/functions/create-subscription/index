...
serve(async (req) => {
    ...
    const log = createLogger("create-subscription", req);
    ...
    try {
        ...
        const { priceId, email, userId, planName, couponCode } = body;

        validateRequired(priceId, "priceId");
        ...
        if (validatedCoupon && finalAmount <= 0) {
            ...
            // Atomically increment coupon used_count before activating the free subscription.
        ...
        // 1. Get or Create Customer. Never accept customerId from the client.
        ...
        let stripeCustomerId = existingCustomerSub?.stripe_customer_id;
        if (!stripeCustomerId) {
            const customer = await stripe.customers.create({
                ...
                idempotencyKey: `customer_${clinicId}`,
            });
        ...
        const idempotencyWindow = Math.floor(Date.now() / 60000);
        const couponKey = validatedCoupon?.id || 'no_coupon';
        const subscriptionIdempotencyKey = `subscription_${clinicId}_${priceId}_${couponKey}_${idempotencyWindow}`;
