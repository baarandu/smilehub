...
export function TrialBanner() {
    ...
    const bgColor = isUrgent ? 'bg-[#fef2f2]' : isWarning ? 'bg-orange-50' : 'bg-blue-50';
    const borderColor = isUrgent ? 'border-[#fca5a5]' : isWarning ? 'border-orange-200' : 'border-blue-200';
    const iconBgColor = isUrgent ? 'bg-[#fee2e2]' : isWarning ? 'bg-orange-100' : 'bg-blue-100';
    const iconColor = isUrgent ? '#DC2626' : isWarning ? '#EA580C' : '#2563EB';
    const textColor = isUrgent ? 'text-[#7f1d1d]' : isWarning ? 'text-orange-800' : 'text-blue-800';
    const subtextColor = isUrgent ? 'text-[#a03f3d]' : isWarning ? 'text-orange-600' : 'text-blue-600';
    const buttonBgColor = isUrgent ? 'bg-[#a03f3d]' : isWarning ? 'bg-orange-600' : 'bg-blue-600';
