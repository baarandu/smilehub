...
import { X, UserPlus, Trash2, Shield, Edit3, Eye, Users } from 'lucide-react-native';
...
const ROLE_CONFIG = {
    admin: { label: 'Administrador', color: '#14b8a6', bgColor: '#ccfbf1' },
    editor: { label: 'Editor', color: '#3b82f6', bgColor: '#dbeafe' },
    viewer: { label: 'Visualizador', color: '#6b7280', bgColor: '#f3f4f6' },
};
...
export function TeamManagementModal({ visible, onClose }: TeamManagementModalProps) {
            ...
            const membersWithProfiles = await Promise.all(
                (data || []).map(async (m) => {
                    ...
                    return {
                        id: m.id,
                        user_id: m.user_id,
                        email: (authUser as any)?.email || `Usuário ${m.user_id.slice(0, 8)}...`,
                        full_name: (profile as any)?.full_name || 'Sem nome',
                        role: m.role as Role,
                    };
                })
            );
    ...
    const handleInvite = async () => {
        ...
        try {
            ...
            const { data: users, error: searchError } = await supabase
                .rpc('get_user_id_by_email', { email_input: inviteEmail.trim().toLowerCase() });
    ...
    const handleChangeRole = (member: TeamMember, newRole: Role) => {
        Alert.alert(
            'Alterar Permissão',
            `Alterar ${member.full_name} para ${ROLE_CONFIG[newRole].label}?`,
            [
                { text: 'Cancelar', style: 'cancel' },
                {
                    text: 'Confirmar',
                                        ...
                                        {member.role !== 'admin' && (
                                            <TouchableOpacity
                                                ...
                                                onPress={() => {
                                                    ...
                                                    handleChangeRole(member, nextRole);
                                                }}
                                            >
