    ...
    return (
        <View className="flex-1">
            <View className="flex-row items-center justify-end px-4 mt-4 mb-2">
                <TouchableOpacity onPress={() => { setTempFilters(activeFilters); setFilterModalVisible(true); }} className={`p-2 rounded-lg border ${activeFilterCount > 0 ? 'bg-[#fef2f2] border-[#fca5a5]' : 'bg-white border-gray-200'}`}>
                    <View className="flex-row items-center gap-2">
                        <Filter size={18} color={activeFilterCount > 0 ? '#EF4444' : '#6b7280'} />
                        <Text className={`text-sm font-medium ${activeFilterCount > 0 ? 'text-[#a03f3d]' : 'text-gray-600'}`}>Filtros</Text>
                        {activeFilterCount > 0 && <View className="bg-[#b94a48] rounded-full w-5 h-5 items-center justify-center"><Text className="text-white text-[10px] font-bold">{activeFilterCount}</Text></View>}
                    </View>
                </TouchableOpacity>
            </View>
            ...
            <ScrollView className="flex-1 px-4 py-2" showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing || false} onRefresh={onRefresh} colors={['#EF4444']} tintColor="#EF4444" />}>
                <View className="flex-row gap-3 mb-4">
                    <View className="flex-1 bg-white p-3 rounded-xl border border-[#fecaca]"><Text className="text-gray-500 text-xs">Manuais</Text><Text className="text-xl font-bold text-[#b94a48] mt-1">{formatCurrency(totalManualExpenses)}</Text></View>
                ...
                <View className="bg-white p-4 rounded-xl border border-[#fca5a5] mb-6 shadow-sm">
                    <View className="flex-row justify-between items-center">
                        <View><Text className="text-gray-500 text-sm">Total de Despesas</Text><Text className="text-3xl font-bold text-[#a03f3d] mt-1">{formatCurrency(totalExpenses)}</Text></View>
                        <View className="w-12 h-12 bg-[#fee2e2] rounded-xl items-center justify-center"><TrendingDown size={24} color="#DC2626" /></View>
                    </View>
                </View>
                        ...
                        <View className="gap-2">
                            {automaticDeductions.cardFees > 0 && (
                                <View className="bg-white p-3 rounded-xl border border-orange-100 flex-row items-center justify-between">
                                    ...
                                    <Text className="font-bold text-[#a03f3d]">- {formatCurrency(automaticDeductions.cardFees)}</Text>
                                </View>
                            )}
                            {automaticDeductions.taxes > 0 && (
                                <View className="bg-white p-3 rounded-xl border border-orange-100 flex-row items-center justify-between">
                                    ...
                                    <Text className="font-bold text-[#a03f3d]">- {formatCurrency(automaticDeductions.taxes)}</Text>
                                </View>
                            )}
                            {Object.entries(automaticDeductions.locationBreakdown).map(([location, amount]) => (
                                <View key={location} className="bg-white p-3 rounded-xl border border-orange-100 flex-row items-center justify-between">
                                    <View className="flex-row items-center gap-3"><View className="w-10 h-10 bg-[#fee2e2] rounded-lg items-center justify-center"><MapPin size={18} color="#b94a48" /></View><View><Text className="font-medium text-gray-900">Taxa do Local</Text><Text className="text-xs text-gray-500">{location}</Text></View></View>
                                    <Text className="font-bold text-[#a03f3d]">- {formatCurrency(amount)}</Text>
                                </View>
                            ))}
                        </View>
                    </View>
                    ...
                    <View className="mb-6">
                        ...
                        <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-row gap-3">
                            {Object.entries(expensesByCategory).map(([cat, amount]: [string, number]) => (
                                <View key={cat} className="bg-white p-3 rounded-lg border border-gray-100 mr-2 min-w-[120px]"><Text className="text-xs text-gray-500 mb-1" numberOfLines={1}>{cat}</Text><Text className="text-lg font-bold text-[#a03f3d]">{formatCurrency(amount)}</Text></View>
                            ))}
                        </ScrollView>
                    </View>
                )}
                                ...
                                <View className="p-4 border-b border-gray-50 flex-row items-center justify-between bg-white">
                                    <View className="flex-1 mr-4">
                                        <View className="flex-row items-start gap-3">
                                            <View className={`w-10 h-10 rounded-lg items-center justify-center ${transaction.category === 'Materiais' ? 'bg-orange-100' : 'bg-[#fee2e2]'}`}>
                                    ...
                                    <View className="flex-row items-center gap-2">
                                        <Text className="font-semibold text-[#b94a48] whitespace-nowrap">- {formatCurrency(transaction.amount)}</Text>
